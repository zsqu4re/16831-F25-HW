...
class PGAgent(BaseAgent):
    ...
    def calculate_q_vals(self, rewards_list):
        ...
        if not self.reward_to_go:
            ...
            #     q_values += [discounted_return] * len(rewards) 
        ...
        # Estimate Q^{pi}(s_t, a_t) by the discounted sum of rewards starting from t
            ...
            #     q_values += list(discounted_cumsums) 
    ...
    def estimate_advantage(self, obs, rewards_list, q_values, terminals):
        ...
        if self.nn_baseline:
            ...
            values = unnormalize(values_normalized, np.mean(q_values), np.std(q_values)+1e-8)
    ...
    def _discounted_return(self, rewards):
        ...
        for t in range(len(rewards)):
            discounted_returns += (self.gamma ** t) * rewards[t]
        discounted_returns = [discounted_returns] * len(rewards)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for t in range(len(rewards)):
            ...
            for t_prime in range(t, len(rewards)):
                discounted_sum += (self.gamma ** (t_prime - t)) * rewards[t_prime]
            discounted_cumsums[t] = discounted_sum
